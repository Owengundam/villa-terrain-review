...
const TerrainEdit=(()=>{
 ...
 /* Start fitting: MOVEMENT-ONLY legalization — slide villas along facade/view axes to
   clear clearance/boundary conflicts, then shrink movements. Orientation is NOT touched
   here (that is terrain-OK's job; re-running it here would duplicate the move). Removal
   is NOT done here — ghosting stays with the view solver.
   cfg: sideGap (m), maxMove (m). Returns new layout units + move log. Every accepted
   move strictly reduces the total issue count, so fitting never worsens the layout. */
 function fitLayout(l,boundary,pts,cfg={}){
  const sideGap=cfg.sideGap??SIDE_GAP,maxMove=cfg.maxMove??12;
