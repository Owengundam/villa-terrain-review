...
const reject=v.edit(l,s.active,s.z,0,false,s.heldOff,v.defaults);assert(!reject.rejected);assert(reject.active[0]);assert(!reject.valid);assert(reject.conflicts.length>0);
...
console.log('PASS protected activation, affected removal, own blockers, unrestricted manual activation, change records and real layout round trips');
