...
const worker=model+`\nonmessage=e=>{try{const {op,layout,active,seed,rules}=e.data;const result=op==='reduce'?View3D.reduce(layout,rules,(iteration,count)=>postMessage({progress:true,iteration,count})):View3D.solve(layout,active,seed,rules);postMessage({result});}catch(e){postMessage({error:String(e)});}};`;
