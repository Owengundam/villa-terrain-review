...
const model=fs.readFileSync('phase0/view3d.js','utf8'),presentation=fs.readFileSync('phase0/villa_presentation.js','utf8'),terrain=fs.readFileSync('phase0/terrain_edit.js','utf8').replace("if(typeof module!=='undefined')module.exports=TerrainEdit;","");
...
const html=fs.readFileSync('phase0/view3d_view.html','utf8').replace('/*__MODEL__*/',model).replace('/*__TERRAIN__*/',terrain).replace('/*__PRESENTATION__*/',presentation).replace('/*__DATA__*/',JSON.stringify(report)).replace('/*__WORKER__*/',JSON.stringify(worker));
