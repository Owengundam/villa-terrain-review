...
const html=fs.readFileSync('phase0/view3d_view.html','utf8').replace('/*__MODEL__*/',model).replace('/*__PARALLEL__*/',parallel).replace('/*__TERRAIN__*/',terrain).replace('/*__PRESENTATION__*/',presentation).replace('/*__DATA__*/',JSON.stringify(report)).replace('/*__WORKER__*/',JSON.stringify(worker));
